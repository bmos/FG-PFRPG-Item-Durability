...
function handleBrokenItem(nodeItem)
	...
	if sItemName ~= '' then
		...
		if nBrokenState == 2 and not sItemName:find('%[DESTROYED%]') then
			...
			ChatManager.Message(string.format(Interface.getString('char_actions_weapon_destroyed'), sItemName), true, rSource);
		elseif nBrokenState == 1 and not sItemName:find('%[BROKEN%]') then
			...
			ChatManager.Message(string.format(Interface.getString('char_actions_weapon_broken'), sItemName), true, rSource);
		else
