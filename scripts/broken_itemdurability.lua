...
function handleBrokenItem(nodeItem)
	...
	local function handleWeaponNodeArgs()
		...
		for _, nodeWeapon in ipairs(DB.getChildList(nodeChar, "weaponlist")) do
			if
				sItemName == DB.getValue(nodeWeapon, "name", "")
				or sUnbrokenItemName == DB.getValue(nodeWeapon, "name", "")
			then
	...
	local function brokenPenalties(bIsBroken)
			...
			if bIsBroken then
				...
				for _, vNode in pairs(tDamagedWeapons) do
					...
					if sItemName:find("%[BROKEN%]") then
						DB.setValue(vNode, "name", "string", string.gsub(sItemName, "%[BROKEN%]%s", ""))
					end
		...
		if
			ItemManager.isShield(nodeItem)
			and StringManager.contains(Extension.getExtensions(), "FG-PFRPG-Advanced-Item-Actions")
		then
			brokenWeapon()
		end
	end
	...
	local function removeBackup()
		for _, nodeName in ipairs({
			...
			if DB.getValue(nodeItem, nodeName .. "bak") then
				DB.deleteChild(nodeItem, nodeName .. "bak")
			end
		end
	end
	...
	elseif nBrokenState == 1 and not sItemName:find("%[BROKEN%]") then
		...
		removeBackup()
		if sItemName:find("%[BROKEN%]") then
			DB.setValue(nodeItem, "name", "string", sItemName:gsub("%[BROKEN%]%s", ""))
		end
	end
end

local function onBrokenChanged(node)
	handleBrokenItem(DB.getParent(node))
end

local function getWeaponName(s)
	local sWeaponName = s:gsub("%[ATTACK%s#?%d*%s?%(%u%)%]", "")
	sWeaponName = sWeaponName:gsub("%[%u+%]", "")
	if sWeaponName:match("%[USING%s") then
		sWeaponName = sWeaponName:match("%[USING%s(.-)%]")
	end
...
local function breakWeapon(nodeWeapon, sWeaponName, rSource)
	if not nodeWeapon or not isFragile(nodeWeapon) then
		return
	end
...
function onFumbleBreakWeapon(rSource, sDesc)
	...
	if string.find(sDesc, "%[CONFIRM%]") or sWeaponName == "" then
		return
	end
