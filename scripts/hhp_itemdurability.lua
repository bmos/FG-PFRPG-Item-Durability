...
local function getSubstanceStats(sItemSubstance)
	for k, v in pairs(ItemDurabilityInfo.aMaterials) do if k == sItemSubstance then return v end end
end
...
function calculateHHP(nodeItem)
	...
	for k, v in pairs(ItemDurabilityInfo.tSizes) do -- luacheck: ignore
		if k == sItemSize then nArmorHpAc = nArmorHpAc * ItemDurabilityInfo.tSizes[k] end
