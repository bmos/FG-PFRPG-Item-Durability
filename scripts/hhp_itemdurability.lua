...
local function getSubstanceStats(sItemSubstance)
    for k, v in pairs(ItemDurabilityInfo.aMaterials) do
        if k == sItemSubstance then return v end
    end
end
...
function calculateHHP(nodeItem)
    ...
    local tSubstanceStats = getSubstanceStats(string.lower(
                                                  DB.getValue(nodeItem,
                                                              'substance', '')))
    if tSubstanceStats then
        for k, v in pairs(tSubstanceStats) do
            if k == 'nHardness' then
                nHardness = v
            elseif k == 'nItemHpPerIn' then
                nItemHpPerIn = v
            elseif k == 'nWeaponHpBonus' then
                nWeaponHpBonus = nWeaponHpBonus + v
            elseif k == 'nWeaponHpMult' then
                nWeaponHpMult = v
            elseif k == 'nArmorHpBonus' then
                nArmorHpBonus = nArmorHpBonus + v
            elseif k == 'nArmorHpMult' then
                nArmorHpMult = v
            end
        end
    end
    ...
    elseif bIsWeapon then
        nItemHp = (nItemHpPerIn * nItemThickness * nWeaponHpMult) +
                      nWeaponHpBonus
    else
        nItemHp = (nItemHpPerIn * nItemThickness)
    end

    DB.setValue(nodeItem, 'hardness', 'number',
                math.floor(nHardness + nHardnessBonus, nil))
    DB.setValue(nodeItem, 'hitpoints', 'number',
                math.floor(nItemHp + nItemHpBonus, nil))
end
